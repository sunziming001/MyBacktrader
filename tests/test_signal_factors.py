...
def test_momentum_is_the_return_over_the_trailing_n_bars(wide):
...
def test_distance_to_high_is_zero_at_a_new_high_and_negative_below_it(wide):
    """距 N 日高点的距离：恰在最高收盘价处为 0，回落则为负——方向是「越大越强」。"""
    prices = wide({"sh600000": [10.0, 12.0, 11.0]})
...
def test_factors_are_comparable_across_symbols_on_the_same_row(wide):
    """同一行的两个数值可直接比大小——这正是排序因子存在的理由。"""
    prices = wide({"sh600000": [10.0, 20.0], "sz000001": [10.0, 15.0]})
...
def test_factors_keep_missing_values_missing_rather_than_filling_zero(wide):
    """缺失不填零：填了就会在截面排序里占到一个具体位置，那是凭空造出的排名。"""
    nan = float("nan")
    prices = wide({"sh600000": [10.0, nan, nan]})
...
def test_factors_are_floats_not_booleans(wide):
    """因子与过滤信号的返回类型是契约的一部分，不可混用（过滤信号见同层测试）。"""
    prices = wide({"sh600000": [10.0, 11.0, 12.0]})
