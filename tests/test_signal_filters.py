...
1. 输出是**纯 ``bool``** 的宽表，缺失一律取 ``False``——语义是「不合格」。
...
def test_new_high_is_strict_so_exactly_matching_the_prior_high_does_not_count(wide):
    """恰好等于前 n 日最高价**不算**新高——边界在此，差一分才算越出。"""
    prices = wide({"sh600000": [10.0, 11.0, 12.0, 12.0, 12.01]})
...
def test_new_high_never_fires_when_the_window_has_no_known_price(wide):
    """全为缺失值的窗口不产生信号，也不被当成「低于现值」而放行（缺口纪律）。"""
    nan = float("nan")
    prices = wide({"sh600000": [nan, nan, nan, nan]})
...
def test_one_missing_bar_inside_the_window_suppresses_the_signal(wide):
    ...
    """
    nan = float("nan")
    prices = wide({"sh600000": [10.0, nan, nan, 20.0]})
...
def test_above_ma_compares_the_close_with_its_own_trailing_average(wide):
    """站上均线：当根收盘高于 n 日均线；窗口未满时不给答案。"""
    prices = wide({"sh600000": [10.0, 11.0, 12.0, 11.0]})
...
def test_ma_cross_up_fires_on_the_crossing_bar_only(wide):
    """均线上穿是**事件**而非状态：越过的那一根为真，其后继续在上方则为假。"""
    prices = wide({"sh600000": [10.0, 9.0, 8.0, 9.5, 9.6]})
...
def test_rising_streak_needs_n_increases_hence_n_plus_one_bars(wide):
    ...
    """
    prices = wide({"sh600000": [1.0, 2.0, 3.0, 4.0, 4.0, 5.0]})
...
def test_volume_surge_is_strict_so_exactly_k_times_the_average_does_not_count(wide):
    ...
    """
    volumes = wide({"sh600000": [100.0, 100.0, 100.0, 200.0, 300.0]})
...
def test_volume_surge_uses_the_prior_average_not_one_including_the_current_bar(wide):
    """基准取自**前 n 根**：含当根会把当根的放量本身算进基准，放量越猛越难触发。"""
    volumes = wide({"sh600000": [100.0, 100.0, 100.0, 400.0]})
...
def test_filters_return_plain_bools_never_missing(wide):
    """过滤器输出纯 ``bool``：缺失被解释为「不合格」，不是「未知」。"""
    nan = float("nan")
    prices = wide({"sh600000": [nan, nan, 12.0, 13.0], "sz000001": [1.0, 2.0, 3.0, 4.0]})
...
def test_a_filter_rejects_a_non_monotonic_index_like_the_indicators_do(wide):
