...
class MarketData:
    ...
    **例外只有一处**：:func:`backward_adjusted_markets` 会拿本类型再包一层，得到一个
    ``prices`` 已是后复权价的同类对象（见那个函数）。那是**视图**，不是新数据，故上面那句
    「经质检行情」对它仍成立；但要留意两件事——它只该喂给算信号与撮合的入口，**不能**拿去
    算估值（PE 与市值要用当时的成交价），而且两者的类型相同，读 ``prices`` 前得先确认手上
    这个是原始价的那个。它的 ``events`` 已清空，正因如此「能不能再折一次」这件事没有第二
    种答案。
