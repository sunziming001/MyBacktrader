"""`vipdoc/cw/gpXX*.dat`：每标的一份的 13 字节定长记录。

.. warning::

    **这一族不是 `GPONEDAT` 的来源。** 票据 #50 早期把两者认作一回事，实测已否：
    `GPONEDAT(ID)` 读的是 ``T0002/hq_cache/gp{sh,sz}one.dat``（10 字节记录、字段号 1..47），
    见 :mod:`mbt.data.gpone`。判定依据是这张表——本族的 ``id = 4`` 的 f1 **精确等于当日
    收盘价**，而帮助原文把 4 指向「一致预期T年度」；本族 ``id = 30`` 的日期则是除权除息日，
    帮助原文指向「机构持股总量」。故**不要**拿 GPONEDAT 的字段表来读这一族。

    本族仍是真实数据（本机 8,976 个文件、1.257 亿条记录），格式已经钉住，但**语义大多未知**，
    故保留于此备将来对账。
