...
.. note::
    ...
    通达信原式里 PEG 还有第三个条件 ``财年T>0``，而 `财年T` 来自 `GPONEDAT(4)`。这个条件是
    **刻意**不在这里实现的，不是遗漏：`财年T` 只能从一致预期快照里读到「现在这一版」，而
    本模块按公告日点取、要保证时点正确（ADR-0006）。把一个最新值混进来会让**整张** PEG 表
    静默失去时点正确性——代价远大于多守一个条件。

    原式的**前瞻分支**（``PE前瞻 = C/EPS_T``、``增预期``、以及 ``用前瞻`` 的两条一起切换）
    实现在 :mod:`mbt.data.forward`，且**只走非回溯入口**（同一理由）。两处分头实现是有意的：
    历史分支要时点正确，前瞻分支做不到，混在一个函数里就会把做不到的那半传染给做得到的那半。
...
def price_earnings_growth(pe: pd.DataFrame, growth_pct: pd.DataFrame) -> pd.DataFrame:
    ...
    .. note::
        ...
        通达信原式里还有第三个条件 ``财年T>0``（`财年T` 来自 `GPONEDAT(4)`）。它**不在**本
        函数里实现：`财年T` 只有「现在这一版」可读（一致预期是快照，无历史，见 ADR-0006
        修订二），而本函数的输入是按公告日点取的时点序列。加它会注入前视偏差，故宁可少守
        一个条件。原式的这一支实现在 :mod:`mbt.data.forward`，且**只能非回溯使用**。
    """
