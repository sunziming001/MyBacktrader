...
- **选股规则：三条现役的**。`Screen` 只提供机制（过滤器 + 排序 + 取前 N），**规则本身**住在
  ...
  3. **砖型**（`brick_screen`，票据 [#85](https://github.com/sunziming001/MyBacktrader/issues/85)）：
     前一天**绿砖**、当天**红砖**、红砖**比那根绿砖大**、**白线在黄线之上**、当天**收在黄线
     之上**、且当天**涨幅 < 5.8%**；按**三项**排序——砖的大小之比、量比、以及当天的 K 线形态
     （**涨幅越小越好**、**上影越短越好**，两条各占一半，合起来与前两项等权，即权重
     `1/3 / 1/3 / 1/6 / 1/6`）。板块范围写在**规则自己**身上（主板 + 创业板 + 科创板，排除
     北交所，`--boards` 可覆盖），且它**只读行情**——不需要 `--cw-root`。「上影」那一项除以
     ATR（窗口 14，与 B1 同口径），白线与黄线用的是**与 B1 同一组参数**（`white_n=10`、
     `DEFAULT_YELLOW_WINDOWS`，见 `CONTEXT.md` 的**白线**与**黄线**）。

     > 「涨幅 < 5.8%」是**不分类别**的固定百分比，故它在各板块含义不同：主板涨停是 +10%
     > （约等于排除涨停），而创业板/科创板是 +20%（那里 +10% 远不是涨停）。要精确排除涨停，
     > 得换成按板块取限幅的涨停判定（`mbt.rules.limit_price` 已有），那是**换判据**而不是换
     > 参数——这一条记在规则 docstring 里，未改。

     > 它的排序因子在 2026-09-19 从**两项等权**改成上面这四项，故**新旧产物不可比**。
     > 那次改动**量过**：两条新因子**没有可用的收益证据**（k=1 是 +0.07pp、k=3/5 是噪声、
     > k=20 反号）。见 [ADR-0017](docs/adr/0017-brick-new-factors-measured-no-evidence.md)。

     > 它那两道**悬趋势**的门（白线在黄线上、收盘在黄线上）**也量过，五处全是负号**——
     > 按天配对筛掉 72.1% 的候选，而**被挡掉的那批之后走得更好**（k=5 起比值 −2.06 / −2.14 /
     > −3.85）。门按需求留着，但**没有证据支持它**，方向上还相反——见
     > [ADR-0018](docs/adr/0018-brick-trend-gates-measured-negative.md)。此前那道门曾是
     > 「收在前若干根的**最高价**之上」，那个更狠（只剩约 6%，且与「红砖比绿砖大」几乎不可能
     > 同时成立），已换掉。
...
| `--panel-bars` | 1301 | **250** | 砖型自己声明要看 114 根（递推记忆约 100 根，而「收在黄线之上」那道门的最长均线要 114 根——取两者之大，见票据 [#86](https://github.com/sunziming001/MyBacktrader/issues/86)），闸门按**它声明的**那个数比。250 是那之上的两倍余量 |
...
取数 108.0 秒、复权 9.3 秒，砖型那一整套读数（**四条过滤器 + 四项因子**，砖型图首算按面板缓存
付一次）
...
面板切到 250 根时峰值提交约 3.4 GB（1301 根是 3.99 GB）——**票里原写的目标是 1.5 GB，实测达不到**，
