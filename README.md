...
- **选股规则：三条现役的**。`Screen` 只提供机制（过滤器 + 排序 + 取前 N），**规则本身**住在
  ...
  3. **砖型**（`brick_screen`，票据 [#85](https://github.com/sunziming001/MyBacktrader/issues/85)）：
     前一天**绿砖**、当天**红砖**、红砖**比那根绿砖大**、且当天**收在黄线之上**；按**三项**
     排序——砖的大小之比、量比、以及当天的 K 线形态（**涨幅越小越好**、**上影越短越好**，两条
     各占一半，合起来与前两项等权，即权重 `1/3 / 1/3 / 1/6 / 1/6`）。板块范围写在**规则自己**
     身上（主板 + 创业板 + 科创板，排除北交所，`--boards` 可覆盖），且它**只读行情**——不需要
     `--cw-root`。「上影」那一项除以 ATR（窗口 14，与 B1 同口径），黄线用的是**与 B1 同一个
     常量** `DEFAULT_YELLOW_WINDOWS`（`(14, 28, 57, 114)`，见 `CONTEXT.md` 的**黄线**）。
     ...
     > 它那道「收在黄线之上」的门也**量过**（`tools/brick_factor_ab.py --mode gate`）：
     > 它筛掉约**三分之二**的候选，而配对差的方向**跨持有期翻转**（k=1 为负、k=3/5 为正、
     > k=10/20 又为负），量级都在 |比值| < 1.5——同样是**没有可用证据**。此前那道门曾是
