...
本机实测一次全跑 **约 22 分钟**（2026-09-16，5,898 个候选、池内 5,449）。阶段拆解：

    ==================  ====   ===================================
    阶段                用时    备注
    ==================  ====   ===================================
    取数                398s   5,898 → 成功 5,449
    算估值信号          227s   5,449 个标的全历史 × 全部财报
    过滤器 volume       275s   旧口径 volume_contraction（ADR-0011）
    过滤器 position     121s   摆动点 + 回调段
    过滤器 trend         48s   白线黄线
    过滤器 low_j         44s   KDJ
    因子 top_calm       101s   整次 volume_pattern 的账（三项共用）
    因子 top_shadow      88s
    其余 4 道过滤器       8s   not_below_the_line / no_flat_pullback / profitable / cheap
    ==================  ====   ===================================

**注意「取数 + 估值」占了一半（625s / 47%）**——时间的大头在**拿数据**，不在算规则。
要压总时长，那里比优化信号层有效。

上表各项之和约 1,310s，实测总时长 1,332s，其余约 22s 是阶段之间的零头与产物落盘。

**别拿干净环境下的单项测量去估这一段**：`volume_pattern` 单独跑全市场是 40s，而在这条
流水线里（跑完 8 道过滤器之后）报的是 **101s**，2.5×。同一现象见 `volume_pattern` 的
docstring：「每有效格」在 3.1~5.7 µs 之间漂。

日志里每个阶段的**实际用时**都会印出来——发现异常慢时先看它，别信进度行里的线性外推
（取数那段的外推尤其离谱：它按前 5 秒的瞬时速率线性外推，早期会给出「还需 78s」这类
与实际差数倍的估计）。
