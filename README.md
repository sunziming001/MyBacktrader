...
- **A 股交易制度约束已基本实现**（票据 [#4](https://github.com/sunziming001/MyBacktrader/issues/4) 收尾中）：
  ...
  - **撮合侧已完成**：**T+1**（当日买入不可当日卖出）、**涨跌停不可成交**、
    **停牌/无成交量不可成交**，且不可成交时挂单**保留至下一可交易日**；
    ...
    跌停一字卖不出但买得到。
  - **尚缺**：行情异常检测（单日涨跌幅超出制度上限应报错，ADR-0005），
...
> **与 ADR-0002 的冲突已收窄到异常检测一处**：[ADR-0002](docs/adr/0002-a-share-trading-constraints.md)
> 决策「回测从第一天就实现 A 股交易制度约束」。费用与撮合（T+1、涨跌停、停牌）现已落地，
> 剩行情异常检测未做（票据 [#4](https://github.com/sunziming001/MyBacktrader/issues/4) 切片 7）；
> 在 #4 完成之前，`run_backtest` 的输出不得用于任何策略优劣的判断。
