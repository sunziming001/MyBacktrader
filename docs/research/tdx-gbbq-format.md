...
## 7. 补记：自研解密的复现结果（2026-09）

按上文的格式描述**自研**实现了解密（`mbt.data.gbbq`，numpy 对记录维度向量化；未移植任何第三方
实现的控制流），并在同一份真实文件上复现了本文的全部关键判断：

1. **长度与计数一致**：5,794,204 B → `(5,794,204 − 4) / 29 = 199,800`，整除精确，无余数。
2. **分组之间无链接模式**：同一只股票的连续记录，密文前 8 字节**逐字节相同**（上文 §2 已列证据）。
   这直接支持两件事——可以按记录维度向量化解密，以及可以从真实文件里**任意截取片段**做 fixture
   （不必整份拷贝，也不受前后记录影响）。
3. **密钥表是格式事实，不是某家实现的产物**：`pytdx` 与 `mootdx2` 各自转抄的 4,176 字节表
   **逐字节相同**（已核对）。自研实现用同一张表，与 `pytdx` oracle 在**全表 199,800 条上逐字段
   一致**（float32 按位比较）。
4. **不依赖 oracle 的自证**：解密后逐条校验 `market ∈ {0,1,2}`、`code` 为 6 位数字、`date` 为
   合法 `YYYYMMDD`、`category ∈ [1,15]`。密钥哪怕错一位都会使绝大多数记录越界——实测全表
   **零违规**。故第 3 条的 oracle 对账是加固证据，而非唯一依据。
5. **类别语义**：类别 1 的 f1..f4 是「每 10 股现金分红 / 配股价 / 每 10 股送转 / 每 10 股配股」，
   与本文 §4 一致；类别 5 的四个浮点数量级确为**万股**股本（本文 §4 的数值核对被复现）。

**结论**：上文的文件结构、加密参数、类别语义与「只有类别 1 驱动价格复权」这一判断，在独立
实现中得到完整复现，可以按已验证对待。密钥表是本项目运行期唯一的格式常量，出处为本文 §8
的来源 2（3DES 风格、4176 字节表、24+5 切分）。

---

## 8. Itemised source list
