...
class ValuationReversal(bt.Strategy, EngineClock):
    ...
    **买入条件不在这里**：它是 :func:`mbt.screen.valuation_screen`（PE 百分位 < 8%、
    ...
    PEG 的增长率取**上一年同期那一期**的同比（``growth_lag_years=1``，见
    :mod:`mbt.data.valuation`）。这一项对结果影响很大：实测茅台在新口径下 PEG 由 ``−9.18``
    变为 ``+2.02``——从「不触发卖出」变成「触发 ``PEG > 1.1`` 卖出」；而 ``sh603901`` 由
    ``0.20`` 升到 ``0.85``，**越过了 0.75 的买入门槛**。
